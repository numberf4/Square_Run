...
local function gameOver()
    ...
    for i=0, 15 do -- We want 15 parts
        ...
        transition.to(dot, {time=300, x=x, y=y, alpha=0})
    end
...
function enterFrame()
    ...
    if _playing then
        ...
        -- Emitter
            ...
            local dot = display.newRect(_grpWorld, _hero.x, _hero.y + 8, size, size)
            ...
            transition.to(dot, {time=300, x=dot.x - x, y=dot.y - y, alpha=10, rotation=-360, xScale=0.7, yScale=0.7, onComplete=function(obj)
                obj:removeSelf()
            end})
...
function scene:create( event )
    ...
    local back1 = display.newImageRect(_grpBackgrounds, "assets/images/background.jpeg", _W, _H)
    back1.x = _CX
    back1.y = _CY
    back1.alpha = 0.3
    ...
    local back2 = display.newImageRect(_grpBackgrounds, "assets/images/background.jpeg", _W, _H)
    back2.x = _CX
    back2.y = _CY - _H
    back2.alpha = 0.3
    ...
    local back3 = display.newImageRect(_grpBackgrounds, "assets/images/background.jpeg", _W, _H)
    back3.x = _CX
    back3.y = _CY - (_H * 2)
    back3.alpha = 0.3
    ...
    _lblTapToStart = display.newText("Start", _CX, _CY, "assets/fonts/Galada.ttf", 46)
    _lblTapToStart.fill = { 1, 1, 1 }
    _grpHud:insert(_lblTapToStart)

    transition.to(_lblTapToStart, {time=2000, y=_CY + 100, iterations=0})
    ...
    _b2 = display.newRect( _grpMain, _blockWidth + _blockWidth * 0.5, _H - 20, _blockWidth, 15)
    _b2.colorID = 2
    _b2.fill = _clrGreen
    _b2.anchorY = 0
    ...
    _b3 = display.newRect( _grpMain, _blockWidth + _blockWidth * 1.5, _H - 20, _blockWidth, 15)
    _b3.colorID = 3
    _b3.fill = _clrBlue
    _b3.anchorY = 0
    ...
    _b4 = display.newRect( _grpMain, _blockWidth + _blockWidth * 2.5, _H - 20, _blockWidth, 15)
    _b4.colorID = 4
    _b4.fill = _clrYellow
    _b4.anchorY = 0
